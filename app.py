...
@app.route('/changeavi')
def changeavi():
    return render_template('changeavi.html')
...
def setAvatar():
    ...
    print ("PIRNT: "+ request.form['avatarpic'])

    POST_AVATAR = str(request.form['avatarpic'])

    users[session["username"]]['avatar'] = POST_AVATAR

    session["avatar"] = users[session["username"]]["avatar"]
    ...
    return render_template('info.html', avatar = avatar)
...
def createUser():
    ...
    if POST_USERNAME in users:
        ...
        users[POST_USERNAME] = {"password" : POST_PASSWORD , "favorites":[] ,"avatar":"https://s.doctoroz.com/recipe/peanut%20butter%20banana%20toast_720.jpg"}
